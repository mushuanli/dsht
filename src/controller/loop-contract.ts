 ...
 * fairly; when the operator supplied a standard with `/verify`, that standard is the rubric.
 ...
 */
export function resultContract(kind: string, limits: LoopLimits, step: number, attempt: number, verification?: string): string[] {
  return [
    '评分与验证：',
    '1. 产出物必须落到工作区（写文件或直接改代码），因为验证者在全新上下文里看不到本对话。',
    '2. 每次尝试都要 spawn 一个全新的 verifier 子代理（subagent，独立上下文），把「原始目标 + 产出物 + 验证标准」交给它独立打分；不要用主回复替代它的判断。',
    '3. 只有当前环境确实没有 subagent 能力时，才允许自评，并在 status 中注明 self-scored。',
    ...(verification === undefined
      ? ['验证标准：未设置（可用 `/verify` 提供）；按原始目标的完成度评分。']
      : ['验证标准（由 /verify 提供，逐条对照）：', verification]),
