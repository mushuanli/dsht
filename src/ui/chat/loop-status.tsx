...
export function LoopStatus({ progress }: { progress: LoopProgress }) {
  ...
  return <Text color={running ? theme.colors.context : theme.colors.muted}>
    {progress.title}{progress.stepLabel === undefined ? '' : ` · ${progress.stepLabel}`} · step {progress.step}/{progress.to} · attempt {progress.attempt}/{progress.tries} · best {progress.best}/{progress.score}{running ? '' : ` · ${progress.phase}`}{progress.note === undefined ? '' : ` · ${progress.note}`}
  </Text>;
}
