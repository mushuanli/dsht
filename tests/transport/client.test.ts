...
test('selected-session interaction replies and reconnection replace the baseline', async t => {
  ...
  controller.actions.pickWorkspace('w1');
  assert.equal(controller.queries.visibleSessions.length, 1);
  await controller.actions.selectSession('s1');
  await until(() => controller.queries.record.ready);
  await controller.actions.prompt('hello');
  ...
  await controller.actions.approve(false);
  ...
  await until(() => controller.state.online && controller.queries.record.ready && controller.state.workspaces.length === 0);
  assert.equal(controller.state.sessionId, 's1');
  assert.equal(controller.queries.record.messages.length, 1);
  assert.equal(fixture.calls.filter(call => call.method === 'session/prompt').length, 1);
});
...
test('workspace and session commands switch across workspaces without creating or cancelling agents', async t => {
  ...
  assert.equal(controller.state.screen, 'workspaces');
  await controller.actions.switchWorkspace('Project α');
  assert.equal(controller.state.workspaceId, 'w1');
  await controller.actions.switchSession();
  assert.equal(controller.state.screen, 'sessions');
  assert.deepEqual(controller.queries.visibleSessions.map(item => item.sessionId), ['s1']);
  await controller.actions.switchSession('all');
  assert.equal(controller.state.showAllSessions, true);
  assert.deepEqual(controller.queries.visibleSessions.map(item => item.sessionId), ['s1', 's2']);
  await controller.actions.switchSession();
  assert.equal(controller.state.showAllSessions, false);
  assert.equal(controller.queries.visibleSessions.length, 1);
  await controller.actions.switchSession('First conversation');
  await until(() => controller.queries.record.ready);
  await controller.actions.switchSession('s2');
...
test('interrupt cancels a running selected session, coalesces repeated keys, and exits only after idle', async t => {
  ...
  controller.start();
  await until(() => controller.queries.record.ready);
  assert.equal(await controller.actions.interrupt(), true);
  ...
  t.after(() => release?.());
  const first = controller.actions.interrupt();
  assert.equal(controller.actions.interrupt(), first);
...
test('Ctrl+C during prompt admission waits for admission before sending cancellation', async t => {
  ...
  t.after(() => release?.());
  const prompt = controller.actions.prompt('work');
  await until(() => release !== undefined);
  const interrupt = controller.actions.interrupt();
...
test('replayed questions survive startup, picker navigation and reconnect without declining the request', async t => {
  ...
  fixture.disconnect();
  await until(() => !controller.state.online);
  await until(() => controller.state.online && controller.queries.record.ready && controller.state.pending.length === 1);
  assert.equal(fixture.calls.some(call => call.method === '$events/result'), false);
  await controller.actions.answer({ answers: [{ id: 'q1', selected: ['Review first'] }] });
  assert.equal(controller.state.pending.length, 0);
