...
| verifier 知道它在评审哪一份文档 | ✅ 已实现 | `verdictBrief` 带上本次 run 解析后的记录变量（`path=…`）并要求产出物属于同一对象；用例：`loop-protocols.test.ts`（默认值与 `vars` 覆盖各一条）、`loop-contract.test.ts`（prompt 里出现 `path=loop.md`）。**未做**：`artifactMarker` 仍不含文档名 |
