...
| 源码规模 | `src/` 57 个模块（8 个业务域 + 共享契约），约 5,810 行；`tests/` 23 个测试文件；159 项测试 |
...
`src/` 共 57 个模块、5,810 行。跨模块消费者通过每个域的 `index.ts` 导入。
...
| `ui/chat/status.tsx` | 225 | `StatusBar`、`elapsedTime`、`metricLines`、`compactStatus` |
