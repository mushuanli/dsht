...
| 源码规模 | `src/` 57 个模块（8 个业务域 + 共享契约），约 5,817 行；`tests/` 24 个测试文件；164 项测试 |
...
`src/` 共 57 个模块、5,817 行。跨模块消费者通过每个域的 `index.ts` 导入。
