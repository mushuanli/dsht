...
- **状态是每会话一行汇总**（会话总额、窗口内每天一个分桶、cut、规则版本、价目表摘要）；session／day／week／month 都由这份状态回答，不再需要汇总表。
- **保留窗口 60 个北京自然日**（`COST_WINDOW_DAYS`）：折叠时更早的日子直接不写；启动时两条规则清理——已加载切片"最新一天已滑出窗口"即删，以及任何 64-hex 账本文件（含**旧代数**与**无法解析**的）只要文件时间早于窗口起点就删。第二条专治"版本升级后旧文件永远不被重写"的残留：切片总是在它所记录的最后一天之后写成，所以按时戳删除不可能删掉窗口内的数据。切片是投影，删掉只值一次重扫，因此目录不再随见过的会话数无限增长——`/loop` 每轮验证 fork 一个会话，这条路径尤其需要它。
- **扫描按窗口过滤**：只翻「运行中 / `updatedAt` 落在窗口内 / 当前选中」三种会话的历史。判据可证明完整——任何计费活动都会推进 `updatedAt`，所以离线期间发生的用量必然落在窗口内；这条恰好补上"断连期间无法用已记录的 `updatedAt` 证明没有用量"（§9 的恢复路径），因此连接世代开始时不再需要无条件全量重读。窗口内仍保留同世代内 `updatedAt` 未变即跳过的优化。
