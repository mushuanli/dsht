...
The rendered rows are identical to a whole-text wrap for every delta. `tests/session/live-wrap.test.ts` streams deterministic random text, including whitespace-only and newline deltas, over widths from 16 to 100, compares the layout against a one-shot wrap after each frame, and fails if a live part loses the identity the incremental path needs. `npm run bench:history` streams 9,600 characters in 400 frames: wrapping the whole text on every frame costs 3,115 ms, the incremental layout 82 ms, and the per-frame cost no longer grows with the accumulated answer. Retained state stays bounded by the finalized rows the layout already cached plus one carried row.
