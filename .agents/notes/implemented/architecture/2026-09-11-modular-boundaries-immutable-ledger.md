...
`app.tsx` keeps the UI state machine and keyboard routing, while presentation and command knowledge moved out: `ui/commands/registry.ts` owned the command catalog, completion and suggestions and `ui/commands/parse.ts` classified one submission into an action (both moved to `slash/` on 2026-09-15), `ui/dialogs/` owns the picker, the panels and the cost panel, `ui/chat/` owns the header, viewport and status bar, `ui/input/` owns the composer, recall, mouse and reference menu, and `ui/mount.tsx` is the only module that renders through Ink.

`tests/architecture/dependencies.test.ts` enforces the boundary: each unit may import only the units listed for it, React and Ink may appear only under `ui/`, and `ui/` may not call the transport client directly. The rule set was tightened on 2026-09-15 to nine forbidden edges with no exemptions (see `architecture/2026-09-15-layered-boundaries-and-plain-ui-contract`). The package surface is decoupled from the layout through `src/index.ts`, with `@itookit/dsht` resolving to `dist/index.js`, `@itookit/dsht/auth` to `dist/transport/auth.js`, and the `dsht` bin to `dist/cli/index.js`.
