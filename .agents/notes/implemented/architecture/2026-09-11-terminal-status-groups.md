...
Groups are packed by value. In keep order the bar holds the state cluster (`◐ 6:18` while running, `● Ready`, `⏸ <reason>`, `! Offline`, `⚠ Error`), the live phase, the stop hint `^C`, the cost, the context share, the model, its effort, the turn count and the token total. When the width runs out the least valuable group is dropped first; the cost is never dropped but moves to the second row, and the state cluster gives up the phase and the stop hint only below about twenty columns. The cost and the share later gained a second reading of the same value, and the display order was separated from the keep order ([readings](2026-09-12-terminal-status-readings.md)).
