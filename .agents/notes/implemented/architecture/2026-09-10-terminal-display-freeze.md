...
Dialogs and pickers freeze their background header and conversation while preserving dialog interactions. Status freezing applies to chat dialogs; workspace selection, session selection, and host-path entry keep connection notices and status live. Otherwise the initial offline frame survives connection because the picker remains open and the session identity does not change. Copy mode still freezes the complete view on every screen. The status clock also pauses during history scrolling. Help, status, and cost panels require dismissal rather than expiring. Mouse-report effects restore terminal flags when disabled and on teardown.
...
Copy mode requires explicit exit to observe new output. Interactive dialogs can still redraw in response to user actions, results, or errors; copy mode freezes the complete view. Native selection behavior depends on the terminal. Tests cover unchanged frames across clock ticks and background events, continued data reception, recovery on exit, dialog persistence, and paused status snapshots. Startup regressions cover initial connection, disconnect/reconnect across the three non-chat screens, and copy-mode retention until explicit exit.
