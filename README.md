...
| `/design-review [--from N] [--to N] [--score X] [--tries N]` | Run the scored multi-round design review; the client drives the loop until each round passes |
| `/loop <score> <tries> <prompt>` | Wrap any prompt in the same scored loop; `--from`/`--to` add refinement rounds |
| `/verify <criteria|off>` | Set, show or clear the standard the next loop is verified against |
...
`/design-review` runs the ten-round convergence review of this repository as a **client-driven** loop. This client sends the first round's full brief, then reads the score out of the `dsht-review` JSON block the agent leaves at the end of its reply and decides what happens next: a score at or above the passing mark advances to the next round, a lower or missing score costs one attempt, and a round that exhausts its attempt budget stops the run. All four limits are options — `--from` (first round, default 1), `--to` (last round, default 10), `--score` (per-round passing mark, default 8, decimals allowed) and `--tries` (attempts per round, default 10) — so `/design-review --from 3 --score 8.5 --tries 5` resumes at round 3. The agent is asked to scope each reply to exactly one attempt, to keep the JSON block in the assistant text, and to have the score come from an independent verifier subagent. A progress line above the composer shows the round, attempt and best score. `/loop <score> <tries> <prompt>` puts any prompt through the same machinery: the prompt is the whole objective, the reply must still end with the `dsht-loop` block, and the loop repeats it until the score reaches the mark or the attempts run out. It is one step by default; add `--from`/`--to` to run several refinement rounds. `score` may be fractional and `tries` is capped at 10 per step. Every loop reply must now end with the shared `dsht-loop` block carrying `score`, `status` (`done|retry|blocked`) and `evidence`: `blocked` stops the run at once instead of spending the attempt budget. The contract also asks the agent to materialize the artifact in the workspace and to have a **fresh verifier subagent** score it — `/verify <criteria>` supplies the standard that verifier scores against (it belongs to the selected session, `/verify` shows it, `/verify off` clears it), so the producer is not its own judge. `/design-review` uses its own per-round checklist as that verifier's rubric (an operator standard is added on top), appends each round to `DESIGN-REVIEW.md` in the workspace so the verifier can read it, and names the round in the progress line. Without a standard the agent scores against the objective itself, and without subagent support it says so in `status`. Sending an ordinary message, `/cancel`, Esc, Ctrl+C, switching sessions or losing the connection all stop the loop, which is never persisted.
...
| Field | Value |
| --- | --- |
| Name and version | `@itookit/dsht` `0.5.0` |
