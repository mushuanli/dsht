...
| `/new` | 在所选工作区创建会话 |
| `/compact` | 会话空闲时压缩旧历史，显示服务端结果 |
| `/cancel` | 取消当前轮次，保留待处理队列 |
| `/queue` | 查看待处理输入；方向键选择，Enter／d／Delete 删除 |
| `/plan [off|message]` | 进入或退出服务端计划模式 |
...
| `/design-review [--from N] [--to N] [--score X] [--tries N]` | 运行带评分的多轮设计审查，由客户端驱动循环直到每轮达标 |
| `/loop <score> <tries> <prompt>` | 把任意 prompt 套进同一评分循环；`--from`/`--to` 可加多轮打磨 |
| `/verify <criteria|off>` | 设置、查看或清除下一次 loop 的验证标准 |
...
`/design-review` 把十轮收敛式审查做成**客户端驱动**的循环：本客户端先发出第一轮的完整 Brief，然后从 agent 回复结尾的 `dsht-review` JSON 块里读取分数并决定下一步——达到及格线就进入下一轮，低于及格线或没有分数块就消耗一次尝试，某轮用尽尝试次数则停止。四个上限都是参数：`--from`（起始轮，默认 1）、`--to`（结束轮，默认 10）、`--score`（每轮及格线，默认 8，允许小数）、`--tries`（每轮尝试次数，默认 10），因此 `/design-review --from 3 --score 8.5 --tries 5` 可从第 3 轮接着做。提示词要求 agent 每次只做一轮一次尝试、JSON 块必须位于正文、分数应由独立的 verifier 子代理给出。输入框上方显示 round · attempt · best/target 进度。`/loop <score> <tries> <prompt>` 让任意 prompt 走同一套机制：prompt 就是全部目标，回复仍需以 `dsht-loop` 块结尾，循环会重复它直到分数达到及格线或尝试次数用尽；默认单步，加 `--from`/`--to` 可跑多轮打磨。`score` 允许小数，`tries` 每步最多 10。循环回复现在必须以共享的 `dsht-loop` 块结尾，块内包含 `score`、`status`（`done|retry|blocked`）与 `evidence`；`status` 为 `blocked` 时立即停止，不再消耗尝试次数。契约还要求 agent 把产出物落到工作区，并由一个**全新的 verifier 子代理**打分——`/verify <criteria>` 提供该验证者对照的标准（属于当前会话；`/verify` 查看，`/verify off` 清除），因此产出者不再是自己作品的裁判。`/design-review` 会直接用它**本轮的检查要点**作为该验证者的 rubric（若另有 `/verify` 标准则叠加在其上），并把每轮结论追加到工作区的 `DESIGN-REVIEW.md` 供验证者读取，同时在进度行显示轮次主题。未设置标准时按目标本身评分；环境不支持子代理时须在 `status` 中说明。用户发送普通消息、`/cancel`、Esc、Ctrl+C、切换会话或断线都会终止循环；该状态不持久化。
