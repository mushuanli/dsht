...
| `/loop [name\|stop] [score] [tries]` | 用列表选择 `loop.yaml` 记录，修改它启动时用的输入与限制后再运行评分循环；`stop` 结束当前运行 |
