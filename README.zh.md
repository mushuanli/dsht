...
单击本地运行的那一条标题栏——`!` 块，或 `/loop` 运行留下的那一行——会打开它的整屏只读视图；其他位置的单击仍是进入复制模式，待画面冻结后再拖动选择。松开鼠标不会恢复刷新，需按 Esc、Ctrl+S 或 Ctrl+C。终端原生 Shift+拖选可能不向应用发送鼠标事件，此时请先按 Ctrl+S。对话框中可按 Ctrl+S 冻结整个画面并释放鼠标捕获，再进行原生选择。
